...
namespace Tests.Services
{
    ...
    public class DirectoryServiceShould
    {
        ...
        public async Task CascadeDeleteDirectorySubdirectoriesAndNotesFromBinByDate()
        {
            //Arrange
        ...
        [Fact(DisplayName = "Be able to cascade mark directory with all subirectories and notes as not deleted and to restore them from bin to directory")]
        public async Task CascadeRestoreDirectorySubdirectoriesAndNotesFromBinToDirectory()
        {
            //Arrange
            var ns = new NoteService(_nr);
            var ds = new DirectoryService(_nr, _dr, _ur, _ir);
            ...
            var directory7 = new Directory("Directory name7", usr);
            var parentDir = new Directory("Parent Dir", usr);
            var bin = new Directory("Bin", usr);
            ...
            var dirFromDatabase6 = await ds.GetDirectoryByIdAsync(directory6.DirectoryId);
            var dirFromDatabase7 = await ds.GetDirectoryByIdAsync(directory7.DirectoryId);
            var parentDirFromDatabase1 = await ds.GetDirectoryByIdAsync(directory7.DirectoryId);
            ...
            Assert.False(dirFromDatabase1.IsMarkedAsDeleted);
            Assert.False(dirFromDatabase2.IsMarkedAsDeleted);
            Assert.False(dirFromDatabase3.IsMarkedAsDeleted);
            Assert.False(dirFromDatabase4.IsMarkedAsDeleted);
            Assert.False(dirFromDatabase5.IsMarkedAsDeleted);
            Assert.False(dirFromDatabase6.IsMarkedAsDeleted);

            Assert.False(dirFromDatabase7.IsMarkedAsDeleted);

            Assert.False(noteFromDatabase1.IsMarkedAsDeleted);
            Assert.False(noteFromDatabase2.IsMarkedAsDeleted);
            Assert.False(noteFromDatabase3.IsMarkedAsDeleted);
            Assert.False(noteFromDatabase4.IsMarkedAsDeleted);
            Assert.False(noteFromDatabase5.IsMarkedAsDeleted);
            Assert.False(noteFromDatabase6.IsMarkedAsDeleted);
            Assert.False(noteFromDatabase7.IsMarkedAsDeleted);
            Assert.False(noteFromDatabase8.IsMarkedAsDeleted);

            var directories = await ds.GetAllDirectoriesForParticularUserAsync(dirFromDatabase1.User.Id);
            Assert.Equal(9, directories.Count);

            var notes = await ns.GetAllUserNotesByIdAsync(dirFromDatabase1.User.Id);
            Assert.Equal(9, notes.Count);


            var directory = await ds.GetDirectoryByIdAsync(directory1.DirectoryId);
            Assert.Equal(directory.ParentDir.DirectoryId, parentDir.DirectoryId);
